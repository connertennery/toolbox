	...
	return (
		<div class={styles.ToolDisplay}>
			<div class={styles.line}>
				<span>name:</span><a href={tool.src}>{tool.name}</a>
			</div>
			<div class={styles.line}>
				<span>loaded:</span><span>{tool.loaded ? `true` : `false`}</span>
			</div>
			<div class={styles.line}>
				<span>type:</span><span>{tool.type}</span>
			</div>
			<div class={styles.line}>
				<span>{tool.size}kb</span><button onClick={(e) => loadTool(tool)}>load</button>
			</div>
			<div class={styles.line}>
				<span>exports: {Object.keys(tool.exports)?.length ?? `N/A`}</span><button onClick={(e) => {
